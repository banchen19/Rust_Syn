...
//帖子评论添加
#[post("/", format = "application/json", data = "<str>")]
pub fn addforum_comment(str: String) -> HttpGetResponder {
    match from_str::<AddForum>(&str) {
        Ok(_addforum) => {
            let config = CONFIG_VAR
                .lock()
                    ...
                    Ok(forum) => {
                        match getplayer_information(config.clone(), forum.clone().sender) {
                            Ok(_player) => {
                                if player.pw == _addforum.pw {
                                    //TODO 初始化论坛数据
                                    add_forum_comment(config,forum);
                                    null_200_http_get_responder()
                                } else {
                                    null_403_http_get_responder()
                                }
...
// 获取帖子对应的所有评论
#[get("/?<name>&<pw>&<forum_title>")]
pub fn getinformation_all_forum_comment(
    name: Option<String>,
    pw: Option<String>,
    ...
    let name = name.unwrap_or_default();
    let pw = pw.unwrap_or_default();
    let forum_title = forum_title.unwrap_or_default();
    println!("name: {}", name);
    println!("pw: {}", pw);
    println!("forum_title: {}", forum_title);
    ...
    match getplayer_information(config.clone(), name.clone()) {
        Ok(player) => {
            if player.pw == pw{
                match getforumcommentall(config,forum_title) {
                    Ok(forums) => {
                        let json_data = serde_json::to_value(&forums).unwrap();
                        let status = Status::Ok;
                        let message = serde_json::to_string(&Response {
